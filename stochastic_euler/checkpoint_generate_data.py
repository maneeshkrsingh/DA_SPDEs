...
model = Euler_SD(n, nsteps=nsteps, mesh = mesh, dt = dt, noise_scale=0.025, salt=True,  lambdas=True)
...
X0_truth[0].interpolate(sin(8*pi*x[0])*sin(8*pi*x[1])+0.4*cos(6*pi*x[0])*cos(6*pi*x[1])
                        +02.3*cos(10*pi*x[0])*cos(4*pi*x[1]) +0.02*sin(2*pi*x[0])+ 0.02*sin(2*pi*x[1]))
...
for i in range(N_init):
    ...
    # model.phi_fn.project(dt*model.psi0)
    # model.phi_mod_fn.project(dt*model.psi0+dt*model.noise_scale*model.dU_3*dt**0.5)
    # print('phi', np.abs(model.phi_fn.dat.data[:]).max())
    # print('phi_mod', np.abs(model.phi_mod_fn.dat.data[:]).max())
...
for i in range(N_obs):
    ...
    if comm.rank == 0:
        ...
        u_1_noise = np.random.normal(0.0, 0.005, (n+1)**2 ) # mean = 0, sd = 0.05
        u_2_noise = np.random.normal(0.0, 0.005, (n+1)**2 ) 
        print(u_1_noise.max(), u_2_noise.min())
