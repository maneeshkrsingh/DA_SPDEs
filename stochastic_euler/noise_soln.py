...
import os
os.makedirs('../../DA_Results/2DEuler/noise_soln/', exist_ok=True)
...
kappa_inv_sq = fd.Constant(0.0)
#kappa_inv_sq = fd.Constant(1/30**2)
print('cell area', (fd.assemble(cell_area*dx(mesh)))**0.5)
...
for t in range(N_time):
    ...
    print('source noise', np.var(dW.dat.data[:]))
...
# psi_problem = fd.LinearVariationalProblem(Apsi, Lpsi,psi0,bcs=bc1,  constant_jacobian=True)
# sp = {"ksp_type": "cg", "pc_type": "lu",
#               "pc_factor_mat_solver_type": "mumps"}
...
# un = 0.5*(fd.inner(-gradperp(psi0), n_F) + abs(fd.inner(-gradperp(psi0), n_F))) # gives fluxes *into* cell only
# #un = 0.5 * fd.inner(v, n_F)  # gives fluxes *into* cell only#
# print(type(un))
# v_courant = fd.TestFunction(DG0)
# Courant_num = fd.Function(DG0, name="Courant numerator")
# #Courant_num_form = dt*(both(un*v_courant)*(fd.dS_v + fd.dS_h)+ un*v_courant*fd.ds_tb )
# Courant_num_form = dt*(un*v_courant*fd.dx )
# Courant_denom = fd.Function(DG0, name="Courant denominator")
# fd.assemble(One*v_courant*fd.dx, tensor=Courant_denom)
...
# print('Courant number', (Courant.dat.data[:]).max())
