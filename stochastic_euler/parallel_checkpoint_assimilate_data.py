...
N_obs = u_vel.shape[0]
PETSc.Sys.Print(u_vel.shape[1])
# VVOM Function
# u_VOM = Function(model.VVOM) 
u_asm_VOM_out = Function(model.VVOM_out)
# u_VOM_out.interpolate(u_VOM)
# u = u_VOM_out.dat.data_ro.copy()

#v_true = model.obs().dat.data[:]
# v_true = u_VOM_out.dat.data_ro.copy()
if jtfilter.subcommunicators.comm.rank == 0:
    print(u_asm_VOM_out.dat.data_ro.shape)
...
for k in range(N_obs):
    ...
    if jtfilter.subcommunicators.comm.rank == 0:
        u_asm_VOM_out.dat.data_wo[:,0] = u_vel[k,:,0]
        u_asm_VOM_out.dat.data_wo[:,1] = u_vel[k,:,1]
        PETSc.Sys.Print('Before assimilation',u_asm_VOM_out.dat.data_ro[:,0])
    # if jtfilter.subcommunicators.comm.rank == 1:
    #     PETSc.Sys.Print('out of rank Before assimilation',u_VOM_out.dat.data_ro[:,0])
    ...
    for step in range(nsteps):
        for i in range(nensemble[jtfilter.ensemble_rank]):
            ...
            fwd_simdata2 = u[:,1]
            #if jtfilter.subcommunicators.comm.rank == 0:
            PETSc.Sys.Print('Before assimilation', fwd_simdata1)
            for m in range(u_vel.shape[1]):
                if jtfilter.subcommunicators.comm.rank == 0:
    ...
    jtfilter.assimilation_step(u_asm_VOM_out, log_likelihood)
...
#         for m in range(u_vel.shape[1]):
#             if jtfilter.subcommunicators.comm.rank == 0:
#                     #PETSc.Sys.Print('After assimilation', u1_sim_list[m].dlocal[i].shape,fwd_simdata1[m].shape)
