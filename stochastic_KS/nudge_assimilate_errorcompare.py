...
filename = f"../../DA_KS/checkpoint_files/ensemble_nudge_250{erank}.h5"
with fd.CheckpointFile(filename, 'r', comm=jtfilter.subcommunicators.comm) as afile:
     for i in range(nensemble[erank]):
         u = jtfilter.ensemble[i][0]
         u0 = afile.load_function(mesh, "nudge250u", idx = i) # read locally
         u.interpolate(u0)
...
for i in range(nensemble[jtfilter.ensemble_rank]):
    ...
    outfile.append(VTKFile(f"../../DA_KS/Nudge_250_500/{ensemble_idx}_output.pvd", comm=jtfilter.subcommunicators.comm))
...
if fd.COMM_WORLD.rank == 0:
    ...
    np.save("../../DA_KS/500_error_nudge_ensemble.npy", Err_final)
    np.save("../../DA_KS/500_nudge_ensemble.npy", y_e)

# # # # do checkpointing for future assimilations 
CG2 = fd.FunctionSpace(model.mesh, "CG", 2)
uout = fd.Function(CG2, name="nudge500u")
