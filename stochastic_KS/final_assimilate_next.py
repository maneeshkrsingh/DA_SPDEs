...
filename = f"../../DA_KS/checkpoint_files/ensemble_nudge_sigma1_250{erank}.h5"
...
if fd.COMM_WORLD.rank == 0:
    ...
    y_sim = np.zeros((np.sum(nensemble), N_obs, ys[1]))
...
if fd.COMM_WORLD.rank == 0:
    np.save("../../DA_KS/500nudgetemp_sigma1_count.npy", np.array(temp_count))

if fd.COMM_WORLD.rank == 0:
   
    print("ensemble shape", y_e.shape)
    if not nudging:
        np.save("../../DA_KS/500Unext_new_tempjitt_assimilated_ensemble.npy", y_e)
    if nudging:
