...
truth_init = VTKFile("../../DA_Results/2DEuler_mixed/paraview_stream/truth_init.pvd")
...
n_x = 16    # partion in x,y direction
params["xpoints"] = n_x
...
N_init = 250  # spinup steps

N_obs = 50 # observation steps
params["N_obs"] = N_obs
...
noise_var = 0.001**2  # Observation noise variance
...
for i in fd.ProgressBar("").iter(range(N_obs)):
    ...
    if comm.rank == 0:
        ...
        obs_points = int(n_x/4 + 1)**2
        psi_noise = np.random.normal(0.0, noise_var**2, obs_points)  # mean = 0, sd = 0.001
        
        # Scale noise relative to signal magnitude (preserves boundary conditions)
        psi_max = np.abs(psi_true).max()
        psi_obs = psi_true + (psi_true/psi_max) * psi_noise
